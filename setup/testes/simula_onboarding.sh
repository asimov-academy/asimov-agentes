  ...
  case "$1 $2" in
    ...
    "POST /admin/clientes/c1/agentes")
      if grep -q '"canal": *"whatsapp"' <<<"$3"; then
        API_STATUS=201; API_RESPOSTA='{"id":"a5","cliente_id":"c1","nome":"Sofia","canal":"whatsapp","ativo":true,"contatos_permitidos":[],"buffer_segundos":8,"max_mensagens_por_resposta":3,"modelo_conversa":"openai:gpt-5.5","modelo_fallback":null,"modelo_auxiliar":"openai:gpt-5.5","modelo_visao":"openai:gpt-5-mini","modelo_transcricao":"openai:whisper-1","handoff_destino":{"tipo":"numero","telefone":"5511977776666","template":{"nome":"aviso_handoff","idioma":"pt_BR"}},"retomada_automatica_horas":4,"digitacao_caracteres_por_segundo":6,"digitacao_maximo_segundos":20,"ferramentas":[],"arquivo_prompt":"loja-exemplo/sofia/persona.md","url_webhook":"https://bot.exemplo.com.br/webhook/whatsapp/tok5","credenciais":{"waba_id":"220011","app_id":"770011","phone_number_id":"1099","numero":"+55 11 3333-4444","access_token":"***","app_secret":"***"}}'; echo "$3" >"$DIR/criado_whatsapp"
    ...
    "PATCH /admin/clientes/c1/agentes/a5") API_STATUS=200; API_RESPOSTA=$(jq -c --argjson m "$3" '. + $m' <<<'{"id":"a5","cliente_id":"c1","nome":"Sofia","canal":"whatsapp","ativo":true,"contatos_permitidos":[],"retomada_automatica_horas":4,"handoff_destino":null}'); echo "$3" >"$DIR/patch_whatsapp" ;;
...
AGENTE='{"id":"a5","cliente_id":"c1","nome":"Sofia","canal":"whatsapp","ativo":true,"contatos_permitidos":[],"retomada_automatica_horas":4,"handoff_destino":{"tipo":"numero","telefone":"5511977776666","template":{"nome":"aviso_handoff","idioma":"pt_BR"}}}'
