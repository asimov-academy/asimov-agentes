...
- `v0.21.3` em `asimov-academy/asimov-agentes` (público): Configurações em um cartão só (eram dois, e o da direita ficava metade vazio), sem as duas linhas que não diziam nada. No menu recolhido, o vão sem explicação onde ficavam os títulos dos grupos virou uma linha fina, e o bloco da conta virou só o avatar em vez de uma caixa larga com um ícone no meio.
