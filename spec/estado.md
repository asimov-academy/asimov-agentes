...
- `v0.21.1` em `asimov-academy/asimov-agentes` (público): título de seção compacto num `Cabecalho` compartilhado (era `text-5xl` com subtítulo embaixo e comia um terço da tela), listagem distribuindo informação pela largura (canal, empresa e modelo em Agentes; situação na coluna do meio em Canais) e logo das integrações na paleta do painel, não na cor da marca.
