...
- `v0.24.1` em `asimov-academy/asimov-agentes` (público): a ajuda do passo "sobre" do onboarding passou a dizer o ganho, com o nome da empresa dentro ("Isso melhora a inteligência dele sobre a Creative Lane"), no lugar de "isto vira o prompt dele", que é jargão e não diz por que responder.
- `v0.24.0`: **onboarding do agente refeito na ordem que o operador desenhou**: nome, objetivo, empresa (com site), o que a empresa faz e os ajustes. Foram oito passos para cinco. O canal saiu da criação: todo agente nasce respondendo no painel e no terminal, e conectar a um WhatsApp ou Chatwoot virou um passo depois, na ficha. O passo do "sobre" ganhou o botão **Melhorar com IA**, que pede à IA da instalação para reescrever o texto do operador. A prévia de conversa de mentira saiu, e o fim virou três caminhos, com conversar em primeiro.
...
| 6. Base de conhecimento | **Construída** (2026-09-18): módulo `conhecimento/`, migração `0021`, rotas no `/admin` e no painel, job `ingerir_documento`, ferramenta de busca e a aba Treinamento funcionando em texto, site e documento. 13 testes novos. **Falta validar em VPS real**, com chave de verdade |
| 7. Polimento e distribuição | **Quase inteira** (2026-09-18): backup diário com timer e retenção de 14 dias, checagens finais do setup (memória, disco, proxy da Cloudflare, `.env` em 600) e revisão do `AGENTS.md.tmpl`. Faltam só as duas decisões do operador: o domínio público do setup e o nome definitivo na licença e no banner |
| 8. Painel web do operador | **Construída inteira e no ar em VPS real** (2026-09-18): entrar, visão geral e o painel servido em `app.<dominio>`. Parte 8.1 (acesso, sessão e as duas telas de login em Jinja2) e as **doze etapas** do front (a última é o copiloto) em `frontend/` (React, Vite, Tailwind): visão geral, agentes com onboarding e ficha em popup, canais, chat, contatos e polimento. Falta o operador percorrer na VPS o critério de aceite inteiro, que inclui criar e editar um agente pelo navegador. Plano e decisões em `spec/frontend.md` |
| 9. Conta de IA vinculada e copiloto do painel | **Concluída e validada em VPS real** (confirmado pelo operador em 2026-09-18, v0.25.2): instalação atualizada, login do Claude Code na tela do setup, vínculo reconhecido, painel no ar e o copiloto respondendo pela assinatura. Vínculo no setup (`setup/lib/vinculo.sh`, `asimov ia`), contêiner e worker do copiloto, servidor MCP com as ferramentas da plataforma, rotas `/painel/api/copiloto` e o popup no painel |
| 10. Humanização dos agentes | **Construída inteira** (2026-09-18), as cinco etapas: ritmo com nome (migração `0022`), pausa de leitura antes do digitando, regras de conversa e de empatia em todo turno, o campo "o que ele nunca deve dizer" e a memória do contato (migração `0023`: resumo rolante da conversa e ficha de fatos, entrando no turno marcados como dado). Mais sentimento no turno, transferência por frustração, o aviso de que é uma IA (migração `0024`) e a prova do agente (cinco perguntas de sempre, no painel e no menu). **Falta validar em VPS.** Plano em [docs/plano-humanizacao.md](../docs/plano-humanizacao.md), fase em spec/fases.md. Decidida em 2026-09-18: uma fase, depois da fase 6; aviso de IA nasce desligado, memória do contato nasce ligada |
...
- Instalação guiada: modo de uso, domínio (colado de qualquer jeito), e-mail do SSL, assistente de código, DNS, instalação com retomada, conta de IA (login do CLI escolhido, que liga o copiloto do painel), oferta do painel, primeiro agente (no painel ou no terminal) e resumo. Nada de chave de IA na instalação desde a v0.20.0: a conta de IA é a assinatura do operador, não uma chave de API.
- Conta de IA (`setup/lib/vinculo.sh`, v0.25.0): a instalação entra na conta do Claude Code ou do Codex, e `asimov ia` vincula, troca de assistente, mostra a situação e desvincula. Pular não impede nada: o painel segue sem copiloto. O segredo do login fica onde o CLI oficial guarda; o `.env` registra só que existe vínculo, com qual CLI, em que conta, onde fica a pasta e com que dono ela está.
...
- **Copiloto** (v0.25.0), no mesmo popup grande: o operador pede em português e o copiloto opera a
  plataforma. O motor é o CLI vinculado, rodando pela assinatura do operador num contêiner próprio
  (`copiloto`, com perfil), que fala com a plataforma só pelo servidor MCP de `copiloto/mcp.py`. Ele
  lê e propõe; toda escrita espera o Confirmar do operador e passa pelos mesmos `servico.py`.
