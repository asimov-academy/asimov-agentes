...
- Verificação local na última revisão: 252 testes passando, `shellcheck` sem erro, `simula_onboarding.sh` completo (inclui o fluxo do WhatsApp), conversa no terminal testada num pty com bash 5.
...
- WhatsApp oficial (`setup/lib/whatsapp.sh`, v0.15.0): pede a conta de WhatsApp Business, o token permanente e o segredo do app, lista os números da conta e os templates aprovados, e cria o agente já com o webhook apontado no número. Item "WhatsApp" em Editar agente: confere o número na Meta, troca destino e template do aviso, horas até voltar sozinho e quem o agente atende. Nada sobe na VPS por causa deste canal.
...
O setup não pede para colar URL nenhuma no painel da Meta: a criação do agente aponta o webhook no próprio número.
