...
| 9. Conta de IA vinculada e copiloto do painel | **Construída inteira e na `main`, nada validado em VPS.** Vínculo no setup (`setup/lib/vinculo.sh`, `asimov ia`), contêiner e worker do copiloto, servidor MCP com as ferramentas da plataforma, rotas `/painel/api/copiloto` e o popup no painel. Plano e decisões em `spec/fases.md` (fase 9) e `spec/decisoes.md` |
