...
| Conversar no terminal | `POST /admin/clientes/{cliente_id}/agentes/{agente_id}/terminal` | texto (1 a 4000) e conversa (vazia começa uma nova) | conversa, conversa_id (para retomar) e agendada (false com handoff aberto: grava e o agente não responde) | agente nativo e ativo do cliente (422 se não é nativo); conversa informada precisa ser desse agente (404); grava a mensagem e agenda o buffer, como um webhook |
| Ler conversa do terminal | `GET /admin/clientes/{cliente_id}/agentes/{agente_id}/terminal/{conversa}?depois=` | ids e `depois` (quantas mensagens do agente o terminal já mostrou) | mensagens do agente depois dessa posição, próxima posição, digitando, respondendo (turno em andamento) e handoff aberto (motivo, resumo, código) | agente nativo do cliente; conversa do agente; o lock do turno é lido antes das mensagens: respondendo falso garante que resposta e handoff já estão gravados |
