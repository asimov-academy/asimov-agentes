...
- WAHA: o contêiner sobe na instalação, junto com o resto (`instala_waha`). Com perfil no Compose ele só subia pelo terminal, e ligar o WhatsApp pelo painel morria num contêiner que ninguém tinha subido: a API não fala com o Docker e nunca vai falar. A imagem é atualizada por timer do systemd no host (`deploy/atualiza_waha.sh`), nunca pelo worker: contêiner com socket do Docker é a VPS inteira.
...
- Credencial de CLI montada no contêiner precisa de três coisas juntas: o volume, o `user:` com o dono do arquivo (login é 600 do dono, root em quase toda VPS) e o arquivo de estado que fica fora da pasta (`~/.claude.json`). Contêiner que já existe não pega volume novo: recrie.
- Arquivo versionado que o setup reescreve na VPS (`deploy/caddy/painel.caddy`) volta ao conteúdo do repositório a cada `asimov atualizar`: quem depende dele precisa ser reescrito na atualização (`painel_garante_caddy`).
