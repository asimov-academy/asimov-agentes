 ...
 *
 *  O agente responde de verdade, com o modelo e o prompt dele, e o turno entra no consumo como
 *  qualquer outro. A tela diz isso: teste não é simulação, e uma resposta aqui custa como uma
 *  resposta lá.
 */
...
export function Teste({
  ...
  return (
    <div className="flex flex-col">
      <div className="flex max-h-72 min-h-[12rem] flex-col gap-3 overflow-y-auto border border-borda bg-void p-4">
        {falas.length === 0 && !esperando && (
          <p className="text-sm text-dim">
            Escreva algo e veja {agente} responder. Ele usa o modelo e o prompt de verdade.
          </p>
        )}
        {falas.map((f, i) => (
      ...
      >
        ...
        <Botao tom="acento" pequeno icone="comm-chat" ocupado={esperando} type="submit">
          Enviar
        </Botao>
      </form>
      ...
      <p className="mt-2 text-sm text-dim">
        Conversa de verdade pelo canal de teste: o turno entra no consumo como qualquer outro.
      </p>
    </div>
  );
}
